...
function buildQuiz(questions, rawQuestionsJSON) {
    let HTMLOutput = [];
    let questionCount = 0;
        ...
        if (question.type === "boolean") {
            ...
            if ("box" in question) {
                ...
                if (typeof(question.box) === "number") {
                    ...
                    // Set default sizes
                    let boxString = question.box.replace("${}", "${30}");

                    // Get all of the box lengths by looping through each matched token and adding them into a list
                    let boxWidths = [];
                    for (const match of boxString.matchAll(/(?<=\${)\d+(?=})/g)) {
                        boxWidths.push(match[0]);
                    }
                    // Remove the box width numbers and split it
                    boxString = boxString.replace(/\${\d*?}/g, "${}");
                    boxString = boxString.split("${}");

                    // Loop through each split point including the index
                    for (let i = 0; i < boxString.length; i++) {
                        if (boxString.length - 1 !== i) {
                            boxString[i] += `<input type="box" id="output${i}" size="${boxWidths[i]}">`;
                        }
                    }

                    HTMLOutput.push(`<p>${boxString.join("")}</p>`);
                }
...
async function displayQuizbox(unitNumber, lessonNumber) {
